...
import { DungeonType, DungeonTypeConfig, DEFAULT_MAP_STYLE } from './settings';
...
export class DungeonGeneratorSettingTab extends PluginSettingTab {
    ...
    display(): void {
        ...
        Object.keys(this.plugin.settings.dungeonTypes).forEach(typeKey => {
            const dungeonType = this.plugin.settings.dungeonTypes[typeKey as DungeonType];
    ...
    addListSetting(
        ...
        property: keyof DungeonTypeConfig, 
        name: string, 
        desc: string
    ): void {
        ...
        const updateList = () => {
            ...
            const items = dungeonType[property] as string[];
